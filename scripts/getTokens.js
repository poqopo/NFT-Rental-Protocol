...
async function main() {
  const [lender, renter, kicker] = await ethers.getSigners();
  ...
  await rent.connect(renter).rent(nft.address, 5, 100)
  // await rent.connect(lender).withdrawCollateral(nft.address, 1)
  // await rent.connect(kicker).kick(nft.address, 1)
  // await nft.connect(lender).setApprovalForAll(rent.address, false)
