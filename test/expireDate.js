...
describe("Rent", function () {
  ...
  describe("ListNFT" , function() {
      it('Can list NFT', async function () {
          ...
          await chai.expect(rent.connect(lender).NFTlist(nft.address, 0, owner.address, 10, 10, 10, 10)).not.to.be.reverted
      })

      it('Cannot list NFT if not owner', async function () {
          const { rent, nft, lender, owner } = await loadFixture(deployRentFixture);
          await nft.connect(lender).safeMint()
          await chai.expect(rent.NFTlist(nft.address, 0, owner.address, 10, 10, 10, 10)).to.be.revertedWith(
      ...
      it('Cannot list if it is paused', async function () {
          ...
          await nft.connect(lender).safeMint()
          await chai.expect(rent.connect(lender).NFTlist(nft.address, 0, owner.address, 10, 10, 10, 10)).to.be.revertedWith(
              "system paused"
            );
      })
      ...
      it('Cannot list if it is already rented', async function () {
          ...
          // NFT mint & approve
          await nft.connect(lender).safeMint()
          await rent.connect(lender).NFTlist(nft.address, 0, token.address, 1000, 10, 1, 10)
          ...
          await chai.expect(rent.connect(renter).NFTlist(nft.address, 0, token.address, 10, 10, 10, 10)).to.be.revertedWith(
              "already rented item!"
            );
      })
  ...
  describe("modifyList" , function() {
      ...
      it('Modify expire Duration', async function () {
        ...
        await nft.connect(lender).safeMint()
        await rent.connect(lender).NFTlist(nft.address, 0, token.address, 1000, 10, 1, 10)
        await chai.expect(rent.connect(lender).modifyList(nft.address, 0, 4, 600)).not.to.be.reverted
        const info = await rent.viewRentinfo(nft.address, 0)
  ...
  describe("Rent" , function() {
      ...
      it('cannot rent for expired Rent', async function () {
        ...
        // NFT mint & approve
        await nft.connect(lender).safeMint()
        await rent.connect(lender).NFTlist(nft.address, 0, token.address, 1000, 10, 10, 0)
        await nft.setApprovalForAll(rent.address, true)
        // Token mint & approve
        await token.connect(renter).mint(renter.address, 1000000)
        await token.connect(renter).approve(rent.address, 1e6)
        await chai.expect(rent.connect(renter).rent(nft.address, 0, 20)).to.be.revertedWith(
            "Expired List"
          );
    })
  ...
  describe("returnNFT" , function() {
      it('can return', async function () {
          ...
          // NFT mint & approve
          await nft.connect(lender).safeMint()
          await rent.connect(lender).NFTlist(nft.address, 0, token.address, 1000, 500, 10, 10)
          await nft.setApprovalForAll(rent.address, true)
          // Token mint & approve
          await token.connect(renter).mint(renter.address, 10000)
          await token.connect(renter).approve(rent.address, 1e6)
      ...
      it('Should send collateral to renter, rentfee to lender, platform fee to fee collector', async function () {
          ...
          // NFT mint & approve
          await nft.connect(lender).safeMint()
          await rent.connect(lender).NFTlist(nft.address, 0, token.address, 1000, 5000, 10, 10)
          await nft.setApprovalForAll(rent.address, true)
          // Token mint & approve
          await token.connect(renter).mint(renter.address, 10000)
          await token.connect(renter).approve(rent.address, 1e6)
  ...
  describe("withdrawNFT" , function() {
      it('can withdraw', async function () {
          ...
          // NFT mint & approve
          await nft.connect(lender).safeMint()
          await rent.connect(lender).NFTlist(nft.address, 0, token.address, 1000, 500, 500, 10)
          await nft.setApprovalForAll(rent.address, true)
          // Token mint & approve
          await token.connect(renter).mint(renter.address, 10000)
          await token.connect(renter).approve(rent.address, 1e6)
      ...
      it('Should send collateral, rentfee to lender, platform fee to fee collector', async function () {
          ...
          // NFT mint & approve
          await nft.connect(lender).safeMint()
          await rent.connect(lender).NFTlist(nft.address, 0, token.address, 1000, 5000, 5000, 10)
          await nft.setApprovalForAll(rent.address, true)
          // Token mint & approve
          await token.connect(renter).mint(renter.address, 10000)
          await token.connect(renter).approve(rent.address, 1e6)
  ...
  describe("Upgrade" , function() {
      ...
      it('Can store List info', async function () {
          ...
          const RentV2 = await ethers.getContractFactory("RentERC721V2");
          const rent2 = await upgrades.upgradeProxy(rent.address, RentV2);
          await chai.expect(await rent2.connect(lender).NFTlist(nft.address, 0, owner.address, 10, 10, 10)).to.equal(true)
      })
